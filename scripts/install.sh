#!/usr/bin/env bash
# Install Gullwing into a local venv and link it into ~/.local/bin
set -euo pipefail
...
if ! "$VENV/bin/python" -c "import tkinter" 2>/dev/null; then
    echo ""
    echo "Note: tkinter not found — the desktop UI (gullwing-ui) won't work."
